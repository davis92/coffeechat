package com.example.coffeechat;
	...
	import com.example.coffeechat.pojo.AgeRange;
	import com.example.coffeechat.pojo.Hometown;
	import com.example.coffeechat.pojo.ImageDetail;
	import com.example.coffeechat.pojo.Location;
	import com.example.coffeechat.pojo.LoginData;
	import com.example.coffeechat.pojo.PickUrl;
	import com.example.coffeechat.pojo.UploadImage;
	import com.example.coffeechat.pojo.UserFaceBookInfo;
	import com.example.coffeechat.pojo.UserFaceBookLikeData;
	import com.example.coffeechat.pojo.UserFacebookLikeId;
	import com.example.coffeechat.pojo.UserProfilePick;
	import com.example.coffeechat.pojo.userProFileData;
	import com.example.coffeechat.utility.AlertDialogManager;
	import com.example.coffeechat.utility.ConnectionDetector;
	import com.example.coffeechat.utility.Constant;
	import com.example.coffeechat.utility.GellaryData;
	import com.example.coffeechat.utility.LocationFinder;
	import com.example.coffeechat.utility.LocationFinder.LocationResult;
	import com.example.coffeechat.utility.SessionManager;
	import com.example.coffeechat.utility.Ultilities;
	import com.example.coffeechatchat.db.DatabaseHandler;
	...
	public class LoginUsingFacebook extends Activity implements OnClickListener {
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			if (session == null) {
				...
				logDebug("onCreate savedInstanceState state session    "
						+ session.getState());
		...
		private class GCMRegistration extends AsyncTask<String, Void, Void> // TODO
																			// GCMRegistration
		{
		...
		private void updateView() {
			Session session = Session.getActiveSession();
			// logDebug(" updateView    session"+session.getState());
			// logDebug(" updateView    acces tokene"+session.getAccessToken());
			// logDebug(" updateView    permissoin "+session.getPermissions());
			if (session.isOpened()) {
		...
		// BackGroundTaskForFetchingDataFromFaceBook
		{
			...
			protected Void doInBackground(String... params) {
				try {
					...
					// logDebug("doInBackground  rp "+rp);
					// logDebug("doInBackground  rp "+rp.getStatusLine().getStatusCode());
					if (rp.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
						String queryAlbums = EntityUtils.toString(rp.getEntity());
						logDebug("BackGroundTaskForFetchingDataFromFaceBook   doInBackground fetch all user data Step 1 : "
								+ queryAlbums);
						Gson gson = new Gson();
					...
					e.printStackTrace();
					flagForResponse = false;
					// logDebug("BackGroundTaskForFetchingDataFromFaceBook   doInBackground Throwable "+
					// e);
				}
				return null;
			}
					...
					new Request.Callback() {
						public void onCompleted(Response response) {
							try {
								...
								logDebug("getUserLikes finalResponse "
										+ finalResponse);
								Gson gson = new Gson();
								...
								for (int i = 0; i < likeid.size(); i++) {
									...
									logDebug("getUserLikes userlikes " + userlikes);
								...
								logError("getUserLikes Exception  " + e);
								new BackGroundTaskForLogind().execute(params);
							}
						}
					});
		...
		private class BackGroundTaskForLogind extends AsyncTask<String, Void, Void> {
				...
				try {
					...
					if (mLoginData.getErrFlag() == 0 && mLoginData.getErrNum() == 2) {
						...
						com.appdupe.flamer.utility.Utility
								.addBitmapToSdCardFromURL(mLoginData
										.getProfilePic().replaceAll(" ", "%20"),
										imageFile);
						...
						logError("BackGroundTaskForLogind doInBackground mLoginData"
								+ mLoginData);
					}
				} catch (Exception e) {
					flagForResponse = false; // originally not commented
			...
			protected void onPostExecute(Void feed) {
				...
				if (flagForResponse) {
					try {
						logDebug("BackGroundTaskForLogind doInBackground message"
						...
						if (mLoginData.getErrFlag() == 0
							...
							// true);
							// dataBundle.putBoolean(CommanConstant.Fromsplash,
							// false);
							...
							getProfildPic();
							// Intent intent=new
							// Intent(LoginUsingActivityActivity.this,MainActivity.class);
						...
						logError("BackGroundTaskForLogind   onPostExecute Exception "
								+ e);
						mdialog.dismiss();
			...
			Request request = new Request(session, "/fql", param, HttpMethod.GET,
					new Request.Callback() {
						public void onCompleted(Response response) {
							...
							logDebug("getProfildPic    response  "
									+ response.toString());
		...
		private class BackGroundTaskForDownload extends
			...
			protected Void doInBackground(String... params) {
				try {
					...
					if (poickList != null && poickList.size() > 0) {
						...
						logDebug("BackGroundTaskForDownload doInBackground uploadResponse "
								+ uploadResponse);
					...
					// syntax
					logError("BackGroundTaskForDownload   doInBackground JsonSyntaxException "
							+ ex);
				} catch (Exception e) {
					logError("BackGroundTaskForDownload   doInBackground Exception "
							+ e);
				}
			...
			protected void onPostExecute(Void result) {
				...
				try {
					...
					logError("BackGroundTaskForDownload   onPostExecute  Exception"
							+ e2);
				}
		...
		private class BackGroundTaskForUserProfile extends
			...
			protected void onPostExecute(Void result) {
				...
				try {
					...
					logDebug("BackGroundTaskForUserProfile  onPostExecute  user persional description "
		...
		private void onClickLogin() {
			...
			if (!session.isOpened() && !session.isClosed()) {
				...
				// session.openForRead(new
				// Session.OpenRequest(this).setPermissions(Arrays.asList("user_birthday",
				// "email","user_relationships","user_photos,user_likes","user_relationship_details","read_stream")).setCallback(statusCallback));
				// session.openForRead(new
				// Session.OpenRequest(this).setPermissions(Arrays.asList("user_birthday",
				// "email","user_relationships","user_photos,user_likes","user_relationship_details","read_stream")).setCallback(statusCallback));

				Session.OpenRequest openRequest = new Session.OpenRequest(this);
				List<String> permissions = Arrays.asList("user_birthday", "email",
						"user_relationships", "user_photos,user_likes",
